...
export function renderAppendEntry(date: string, content: string, timestamp: string): string {
...
export async function getLogStats(baseDir = process.cwd()) {
  ...
  const logDaysThisMonth = sortedFileNames.filter(
  (fileName) => fileName.startsWith(currentMonth),
  ).length;
  let currentStreak = 0;
  let longestStreak = 0;
  const logDaysLastMonth = sortedFileNames.filter(
  (fileName) => fileName.startsWith(previousMonth),
  ).length;
  const logRateLastMonth =
  daysInLastMonth === 0
    ? 0
    : (logDaysLastMonth / daysInLastMonth) * 100;
  const logDaysChange = logDaysThisMonth - logDaysLastMonth;
  const logRateThisMonth =
...
export async function writeLogFile(
  ...
  try {
    ...
    if (existingContent.trim() !== "") {
      ...
      await appendFile(
        filePath,
        `${separator}${renderAppendEntry(date, content, getJstTimestampString())}`,
        "utf8",
      );
...
function getMobileImportDateFromFilename(filePath: string): string {
...
async function collectMobileImportPlans(
  importPath: string,
  outputDir: string,
): Promise<MobileImportPlan[]> {
  let targetStat;
...
function buildLogWithMobileNotes(
  existingLog: string | null,
  date: string,
  mobileMarkdown: string,
): string {
  const mobileBody = normalizeLineEndings(mobileMarkdown).trim();
  const initialLog =
    existingLog === null || existingLog.trim() === ""
      ? `# ${date}\n`
...
async function importSingleMobileFile(plan: MobileImportPlan, dryRun: boolean): Promise<void> {
  if (dryRun) {
    return;
  }

  let mobileMarkdown: string;
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Failed to archive mobile file: ${plan.sourcePath} -> ${plan.archivePath}: ${message}`,
    );
  }
}

async function importMobileFiles(
  importPath: string,
  outputDir: string,
  dryRun: boolean,
): Promise<MobileImportPlan[]> {
...
function formatMobileImportPlan(plan: MobileImportPlan, dryRun: boolean): string[] {
...
export async function runCli(
  ...
  if (parsed.advice) {
    ...
    if (stats.latestLog === null) {
      ...
      const recentLogContents = await Promise.all(
      recentFileNames.map(async (fileName) => {
